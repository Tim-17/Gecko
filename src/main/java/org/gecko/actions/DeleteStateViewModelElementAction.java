...
public class DeleteStateViewModelElementAction extends AbstractPositionableViewModelElementAction {
    ...
    boolean run() throws GeckoException {
        wasStartState = automaton.getStartStates().contains(stateViewModel.getTarget());
        if (wasStartState && automaton.getStartStates().size() == 1 && automaton.getStates().size() > 1) {
            ...
            systemViewModel.addStartState(newStartStateViewModel);
        ...
        if (automaton.getStates().size() == 1 && wasStartState) {
            systemViewModel.getStartStates().clear();
        }
        systemViewModel.updateTarget();
